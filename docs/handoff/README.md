...
1. **Migration risk: RESOLVED (Aug 15, 2026).** Export was feared broken on the installed build, but a real export was successfully produced on a family device. A **sanitized copy** (personal names/places/medical notes replaced with placeholders; structure byte-for-byte equivalent otherwise) is checked in at [`fixtures/stackmap-export-2026-08-15-sanitized.json`](./fixtures/stackmap-export-2026-08-15-sanitized.json) as the canonical importer test fixture. The unsanitized original — the actual migration source — stays outside git in the family's own storage. Keep exporting periodically until the new app ships; the contingency plan in [04-backup-spec.md](./04-backup-spec.md#5-migration-off-the-old-app) remains for reference only.
