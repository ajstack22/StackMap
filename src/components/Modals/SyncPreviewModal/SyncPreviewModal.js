...
const SyncPreviewModal = ({
  ...
  const checkSyncData = async () => {
    try {
      ...
      const response = await fetch(
        `${syncService.getApiUrl()}/pull.php?sync_id=${syncId}&device_id=${encodeURIComponent(
          deviceId,
        )}`,
        {
          method: 'GET',
  ...
  const getStatusIcon = () => {
    switch (connectionStatus) {
      case 'checking':
        return (
          <ActivityIndicator size="small" color={theme?.primary || '#5C7E9D'} />
        );
      case 'connected':
