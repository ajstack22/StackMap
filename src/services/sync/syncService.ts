  ...
  async verifySyncExists(): Promise<boolean> {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/pull.php?sync_id=${this.syncId}&device_id=${deviceId}`,
      );
        ...
        Object.keys(userData.days).forEach(day => {
          if (userData.days[day]?.activities) {
            userData.days[day].activities = userData.days[
              day
            ].activities.filter((activity: any) => !activity.deleted);
          }
        });
      }
          ...
          Object.keys(filteredUserData.days).forEach(day => {
            if (filteredUserData.days[day]?.activities) {
              filteredUserData.days[day].activities = filteredUserData.days[
                day
              ].activities.filter((activity: any) => !activity.completed);
            }
          });
        }
      }
  ...
  async getActiveShares(): Promise<ShareInfo[]> {
    try {
      ...
      return processedShares.filter(
        (share: ShareInfo | null) => share !== null,
      );
    } catch (error) {
  ...
  async deleteShare(shareId: string): Promise<boolean> {
    try {
      ...
      const filtered = shares.filter(
        (share: ShareInfo) => share.shareId !== shareId,
      );
...
(syncService as any).generateSyncId =
