...
class ConflictResolver {
  ...
  mergeUsers(localUsers, remoteUsers, localMeta, remoteMeta) {
    ...
    if (remoteUserTime > localUserTime + 3000) {
      this.log(
        `Using remote users (newer by ${
          (remoteUserTime - localUserTime) / 1000
        }s)`,
      );
      return remoteUsers;
    }
    if (localUserTime > remoteUserTime + 3000) {
      this.log(
        `Using local users (newer by ${
          (localUserTime - remoteUserTime) / 1000
        }s)`,
      );
      return localUsers;
    }
    ...
    allUserIds.forEach(userId => {
      ...
      if (!localUser && remoteUser) {
        ...
        merged[userId] = this.mergeIndividualUser(
          localUser,
          remoteUser,
          userId,
        );
  ...
  mergeIndividualUser(localUser, remoteUser, userId) {
    ...
    if (remoteModified > localModified) {
      ...
      this.log(
        `User ${userId}: Using remote as base (${remoteModified} > ${localModified})`,
      );
    } else if (localModified > remoteModified) {
      mergedUser = { ...localUser };
      this.log(
        `User ${userId}: Using local as base (${localModified} > ${remoteModified})`,
      );
    } else {
  ...
  mergeActivitiesArray(localActivities, remoteActivities) {
    ...
    remoteActivities.forEach(activity => {
      if (!existingIds.has(activity.id)) {
        ...
        const localTime =
          localActivity?.modifiedAt || localActivity?.createdAt || 0;
    ...
    allIds.forEach(activityId => {
      ...
      if (!local && remote) {
        ...
        if (remoteTime > localTime) {
          merged[activityId] = remote;
          this.log(
            `Activity ${activityId}: Remote wins (${remoteTime} > ${localTime})`,
          );
        } else if (localTime > remoteTime) {
          merged[activityId] = local;
          this.log(
            `Activity ${activityId}: Local wins (${localTime} > ${remoteTime})`,
          );
        } else {
          // Same timestamp - use device ID as tiebreaker
          const winner = this.tiebreaker(
            localMeta.deviceId,
            remoteMeta.deviceId,
          );
          merged[activityId] = winner === 'local' ? local : remote;
          this.log(
            `Activity ${activityId}: Tie broken by device ID (${winner})`,
          );
        }
      }
    });
  ...
  mergeSettings(localSettings, remoteSettings, localMeta, remoteMeta) {
    ...
    if (remoteTimestamp > localTimestamp) {
      this.log(
        `Settings: Remote wins (${remoteTimestamp} > ${localTimestamp})`,
      );
      return remoteSettings;
  ...
  mergeLibrary(localLibrary, remoteLibrary, localMeta, remoteMeta) {
    ...
    if (
      localLibrary.userAddedActivityIds ||
      remoteLibrary.userAddedActivityIds
    ) {
      ...
      if (merged.userAddedActivityIds.length > localIds.length) {
        this.log(
          `Library: Added ${
            merged.userAddedActivityIds.length - localIds.length
          } user activity IDs from remote`,
        );
      }
    }
  ...
  logSummary() {
    if (this.enableLogging && this.mergeLog.length > 0) {
      ...
      const remoteWins = this.mergeLog.filter(l =>
        l.message.includes('Remote wins'),
      ).length;
      const localWins = this.mergeLog.filter(l =>
        l.message.includes('Local wins'),
      ).length;
      const ties = this.mergeLog.filter(l =>
        l.message.includes('Tie broken'),
      ).length;
