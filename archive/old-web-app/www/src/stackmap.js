        ...
        class StackMapApp {
            constructor() {
                ...
                this.data = {
                    ...
                    globalSettings: {
                        ...
                        editModePin: null, // null = disabled, string = 4-digit PIN
                        pinEnabled: false
                    },
                ...
                wizard.innerHTML = `
                    <div class="setup-content">
                        ...
                        <div class="setup-step" data-step="4">
                            <p>Would you like to protect Edit Mode with a PIN?</p>
                            <p class="setup-hint">This helps prevent accidental changes to your routine</p>
                            <div class="setup-buttons" style="margin-top: 30px;">
                                <button class="setup-btn setup-btn-secondary" onclick="app.skipPinSetup()">
                                    Skip for Now
                                </button>
                                <button class="setup-btn setup-btn-primary" onclick="app.startPinSetup()">
                                    Set PIN
                                </button>
                            </div>
                        </div>
            ...
            initializeCapacitor() {
                ...
                if (typeof Capacitor !== 'undefined' && Capacitor.isNativePlatform()) {
                    ...
                    // Hide splash screen now that app is loaded
                    if (SplashScreen) {
                        SplashScreen.hide();
                    }
            ...
            openPanel(panelType) {
                ...
                if (panel) {
                    this.setupPanelSwipeGesture(panel);
                }
            ...
            setupPanelSwipeGesture(panel) {
                let startY = 0;
                let currentY = 0;
                let isDragging = false;
                
                const handleTouchStart = (e) => {
                    // Only handle touches on the panel header or handle bar
                    ...
                    // Start drag if touching within top 80px of panel (header area)
                    if (touchY - panelTop < 80 || panelHeader?.contains(target)) {
            ...
            renderEditContent() {
                // Always show edit controls directly (PIN check happens before opening panel)
            ...
            getEditControlsHTML() {
                return `
                    ...
                    <!-- PIN Management -->
                    <div class="user-management-section">
                        <label class="day-toggle-label">Edit Mode Security</label>
                        <button class="btn-wide btn-secondary" onclick="app.managePinSettings()" style="width: 100%; margin-top: 10px;">
                            <span class="material-icons">${this.data.globalSettings.pinEnabled ? 'lock' : 'lock_open'}</span>
                            <span>${this.data.globalSettings.pinEnabled ? 'Change PIN' : 'Set PIN'}</span>
                        </button>
                    </div>
                ...
                try {
                    ...
                    if (saved) {
                        ...
                        if (loadedData.version === 3) {
                            ...
                            // Ensure PIN fields exist
                            if (!this.data.globalSettings.hasOwnProperty('editModePin')) {
                                this.data.globalSettings.editModePin = null;
                                this.data.globalSettings.pinEnabled = false;
                            }
            ...
            migrateFromV2(v2Data) {
                ...
                // Ensure PIN fields in globalSettings
                if (!this.data.globalSettings.hasOwnProperty('editModePin')) {
                    this.data.globalSettings.editModePin = null;
                    this.data.globalSettings.pinEnabled = false;
                }
            ...
            showPinEntry(options = {}) {
                ...
                let enteredPin = '';
                const modal = document.createElement('div');
                modal.className = 'pin-entry-modal';
                    ...
                    if (enteredPin.length < 4) {
                        ...
                        if (enteredPin.length === 4) {
                            if (isSetup && !isConfirm) {
                                ...
                                this.showPinEntry({
                                    ...
                                    onSuccess: (confirmedPin) => {
                                        if (confirmedPin === enteredPin) {
                                            this.data.globalSettings.editModePin = enteredPin;
                                            this.data.globalSettings.pinEnabled = true;
                                            this.saveData();
                                            onSuccess(enteredPin);
                                        } else {
                                            this.showToast('PINs do not match. Please try again.', null, null);
                                            this.showPinEntry(options);
                                        }
                                    },
                                    onCancel,
                                    allowYearReset: false
                                });
                            } else if (allowYearReset && this.isValidBirthYear(enteredPin)) {
                                // Year entered - show reset options
                                modal.remove();
                                this.showPinResetOptions();
                            } else if (!isSetup && enteredPin === this.data.globalSettings.editModePin) {
                                // Correct PIN entered
                                modal.remove();
                                onSuccess(enteredPin);
                            } else if (isConfirm) {
                                // Confirm mode - let parent handle validation
                                modal.remove();
                                onSuccess(enteredPin);
                            } else {
                                // Wrong PIN
                                const errorEl = modal.querySelector('.pin-entry-error');
                                if (errorEl) {
                                ...
                                // Shake animation
                                const container = modal.querySelector('.pin-entry-container');
                    ...
                    <div class="pin-entry-container">
                        ...
                        <div class="pin-keypad">
                            ${[1,2,3,4,5,6,7,8,9].map(num => 
                                `<button class="pin-key" onclick="app.tempPinHandler('${num}')">${num}</button>`
                            ).join('')}
                            <button class="pin-key cancel" onclick="app.tempPinCancel()">Cancel</button>
                            <button class="pin-key" onclick="app.tempPinHandler('0')">0</button>
                            <button class="pin-key delete" onclick="app.tempPinDelete()">
                                <span class="material-icons">backspace</span>
                            </button>
                        </div>
                        
                        <div class="pin-entry-error"></div>
                        ${allowYearReset && !isSetup ? 
                            '<p class="pin-entry-info">Forgot PIN? Enter your birth year (must be 13+ years old)</p>' : 
                            ''
                        }
                    </div>
                `;
                ...
                // Temporary handlers
                this.tempPinHandler = handleKeyPress;
                this.tempPinDelete = handleDelete;
                this.tempPinCancel = () => {
                    modal.remove();
                    onCancel();
                };
            ...
            isValidBirthYear(pin) {
                const year = parseInt(pin);
                const currentYear = new Date().getFullYear();
                const age = currentYear - year;
                return year >= 1900 && year <= currentYear && age >= 13;
            }
            ...
            showPinResetOptions() {
                ...
                modal.innerHTML = `
                    <div class="pin-entry-container">
                        <h2 class="pin-entry-title">Reset PIN Options</h2>
                        <p class="pin-entry-subtitle">What would you like to do?</p>
                        
                        <div style="display: flex; flex-direction: column; gap: 15px; margin-top: 30px;">
                            <button class="btn btn-primary" onclick="app.tempResetPin()" style="width: 100%; padding: 15px;">
                                <span class="material-icons" style="vertical-align: middle; margin-right: 8px;">lock</span>
                                Set New PIN
                            </button>
                            <button class="btn btn-secondary" onclick="app.tempDisablePin()" style="width: 100%; padding: 15px;">
                                <span class="material-icons" style="vertical-align: middle; margin-right: 8px;">lock_open</span>
                                Disable PIN
                            </button>
                            <button class="btn btn-secondary" onclick="app.tempCancelReset()" style="width: 100%; padding: 15px;">
                                Cancel
                            </button>
                        </div>
                    </div>
                ...
                this.tempResetPin = () => {
                    ...
                    this.showPinEntry({
                        ...
                        isSetup: true,
                        onSuccess: () => {
                            this.showToast('PIN updated successfully', null, null);
                        },
                        allowYearReset: false
                    });
                };
                ...
                this.tempDisablePin = () => {
                    ...
                    this.saveData();
                    modal.remove();
                    this.showToast('PIN has been disabled', null, null);
                };
                
                this.tempCancelReset = () => {
                    modal.remove();
                };
            ...
            managePinSettings() {
                if (this.data.globalSettings.pinEnabled) {
                    // Show reset options directly
                    this.showPinResetOptions();
                } else {
                    // Start PIN setup
                    this.showPinEntry({
                        ...
                        isSetup: true,
                        onSuccess: () => {
                            this.showToast('PIN set successfully', null, null);
                            this.renderEditContent();
                        },
                        allowYearReset: false
                    });
            ...
            checkPinBeforeEdit() {
                if (this.data.globalSettings.pinEnabled && this.data.globalSettings.editModePin) {
                    this.showPinEntry({
                        title: 'Enter PIN for Edit Mode',
                        subtitle: 'Enter your 4-digit PIN to access edit mode',
                        onSuccess: () => {
                            this.openPanel('edit');
                        }
                    });
