...
(function() {
    ...
    const CompleteDayWorkflow = {
        ...
        processActivities: function(activities) {
            ...
            // Step 3: Process activities based on type and pin status
            const activitiesToKeep = [];
            const carryForwardActivities = [];
            ...
            activities.forEach(function(activity) {
                ...
                // Handle pinned activities based on pin type
                if (activity.pinned && activity.pinType) {
                    switch (activity.pinType) {
                        case 'daily':
                            // Daily pins: Reset completion status, keep in today
                            activity.completed = false;
                            activity.status = 'pending';
                            if (activity.metadata) {
                                delete activity.metadata.completedAt;
                            }
                        ...
                        case 'carry-forward':
                            // Carry-forward pins: Clone to tomorrow
                            if (!isCompleted) {
                                // If not completed, move to tomorrow
                                const carriedActivity = Object.assign({}, activity);
                                carriedActivity.id = 'activity_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
                                carriedActivity.day = 'tomorrow';
                                carriedActivity.timeframe = 'tomorrow';
                                carriedActivity.created = Date.now();
                                carriedActivity.modified = Date.now();
                                carryForwardActivities.push(carriedActivity);
                            }
                            // Original stays in today as completed (or is removed if completed)
                            if (!isCompleted) {
                                activitiesToKeep.push(activity);
                            }
                            break;
                        ...
                        case 'permanent':
                            // Permanent pins: Never complete, always keep
                            activity.completed = false;
                            activity.status = 'pending';
                            if (activity.metadata) {
                                delete activity.metadata.completedAt;
                ...
                } else if (activity.pinned) {
                    // Legacy pinned activities without pinType - treat as daily
                    if (!isCompleted) {
                        activity.completed = false;
                        activity.status = 'pending';
                        activitiesToKeep.push(activity);
            ...
            // Step 4: Add new tomorrow activities and carry-forward activities
            const updatedActivities = activitiesToKeep.concat(newTomorrowActivities).concat(carryForwardActivities);
            ...
            console.log('CompleteDayWorkflow: Carried forward', carryForwardActivities.length, 'pinned activities');
            console.log('CompleteDayWorkflow: Final activity count:', updatedActivities.length);
            
            // Log pin summary for debugging
            const pinSummary = {
                daily: activities.filter(a => a.pinned && a.pinType === 'daily').length,
                carryForward: activities.filter(a => a.pinned && a.pinType === 'carry-forward').length,
                permanent: activities.filter(a => a.pinned && a.pinType === 'permanent').length
            };
            console.log('CompleteDayWorkflow: Pin summary:', pinSummary);
        },
