...
(function() {
    ...
    const ActivityPin = {
        ...
        pinActivity: function(activity, pinType) {
            ...
            // Set timestamps
            if (!activity.pinCreatedAt) {
                activity.pinCreatedAt = now;
            }
            activity.lastPinTypeChange = now;
            activity.modified = now;
            
            console.log(`ActivityPin: Pinned activity "${activity.title}" as ${activity.pinType} pin`);
            
            // Save the change
            self.saveActivity(activity);
            
            // Update UI immediately
            self.updatePinButtonState(activity.id, true);
            self.updateActivityVisualState(activity.id, true);
            
            // Announce to screen readers
            if (window.StackMapKeyboardNav && window.StackMapKeyboardNav.announce) {
                const message = `${activity.title} pinned as ${pinType} pin`;
         ...
         * Unpin an activity
         */
        unpinActivity: function(activity) {
            const self = this;
            
            // Remove pin properties
            activity.pinned = false;
            delete activity.pinType;
            delete activity.pinCreatedAt;
            delete activity.lastPinTypeChange;
            activity.modified = Date.now();
            
            console.log(`ActivityPin: Unpinned activity "${activity.title}"`);
            
            // Save the change
            self.saveActivity(activity);
            
            // Update UI immediately
            self.updatePinButtonState(activity.id, false);
            self.updateActivityVisualState(activity.id, false);
            
            // Announce to screen readers
            if (window.StackMapKeyboardNav && window.StackMapKeyboardNav.announce) {
                const message = `${activity.title} unpinned`;
                window.StackMapKeyboardNav.announce(message);
            }
