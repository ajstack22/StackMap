...
class MobileUXEnhancements {
  ...
  initializeMobileEnhancements() {
    ...
    // Set up periodic cleanup for ghost cards
    setInterval(() => this.cleanupGhostCards(), 2000);
    
    // Apply header position preference
    this.applyHeaderPositionPreference();
  }
  ...
  handleTouchEnd(e) {
    ...
    // Clean up any orphaned drag clones
    this.cleanupGhostCards();
  ...
  cleanupGhostCards() {
    // Remove any lingering drag clones that might be stuck
    document.querySelectorAll('.card--dragging').forEach(clone => {
      if (clone.style.position === 'fixed' && clone.style.pointerEvents === 'none') {
        clone.remove();
      }
    });
    
    // Also clean up any cards stuck in drag-ready state
    document.querySelectorAll('.drag-ready').forEach(card => {
  ...
  applyHeaderPositionPreference() {
    // Get saved preference or default to top
    const headerPosition = localStorage.getItem('stackmap-mobile-header-position') || 'top';
    
    if (headerPosition === 'bottom') {
      document.body.classList.add('mobile-header-bottom');
    } else {
      document.body.classList.remove('mobile-header-bottom');
    }
  }
  
  setHeaderPosition(position) {
    // Save preference
    localStorage.setItem('stackmap-mobile-header-position', position);
    ...
    // Update the segmented control UI
    const control = document.querySelector('[data-control="header-position"]');
    if (control) {
      control.querySelectorAll('.segment').forEach(btn => {
        const isActive = (position === 'top' && btn.textContent.includes('Top')) ||
                        (position === 'bottom' && btn.textContent.includes('Bottom'));
        btn.classList.toggle('segment--active', isActive);
        btn.setAttribute('aria-pressed', isActive);
      });
    }
    
    // Trigger a render to adjust any dynamic elements
    if (window.appInstance) {
      window.appInstance.render();
    }
  }
  
  createHeaderPositionToggle() {
    // Create the settings UI for header position using segmented control style
    const currentPosition = this.getHeaderPosition();
    
    const settingHtml = `
      <div class="segmented-control" data-control="header-position">
        <button class="segment ${currentPosition === 'top' ? 'segment--active' : ''}" 
                onclick="window.mobileUX.setHeaderPosition('top')"
                aria-pressed="${currentPosition === 'top'}">
          <span class="material-icons">vertical_align_top</span>
          <span>Top</span>
        </button>
        <button class="segment ${currentPosition === 'bottom' ? 'segment--active' : ''}" 
                onclick="window.mobileUX.setHeaderPosition('bottom')"
                aria-pressed="${currentPosition === 'bottom'}">
  ...
  getHeaderPosition() {
    return localStorage.getItem('stackmap-mobile-header-position') || 'top';
  }
