...
const VoiceAttachment = (function() {
  'use strict';
  ...
  VoiceAttachment.prototype.save = function(taskId, blob, metadata, callback) {
    ...
    return this.getCountForTask(taskId).then(function(count) {
      ...
      const sql = 
        `INSERT INTO ${self.tableName} (task_id, blob, mime_type, duration, size, mode, created_at, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  VoiceAttachment.prototype.exportForTask = function(taskId, callback) {
    ...
    return this.getForTask(taskId).then(function(attachments) {
      ...
      attachments.forEach(function(attachment) {
        const promise = self.getBlob(attachment.id).then(function(blob) {
          exports.push({
            metadata: attachment,
            blob: blob
          });
