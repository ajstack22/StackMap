    ...
    var TaskSQLite = {
        ...
        createTables: function() {
            ...
            var statements = [
                ...
                // Key-value storage table for general data
        ...
        getItem: function(key, callback) {
            ...
            self.sqlite.query({
                database: self.dbName,
                statement: 'SELECT value FROM storage WHERE key = ?',
                values: [key]
            }).then(function(result) {
                if (result.values && result.values.length > 0) {
                    try {
        ...
        setItem: function(key, value, callback) {
            ...
            self.sqlite.run({
                database: self.dbName,
                statement: 'INSERT OR REPLACE INTO storage (key, value, updated_at) VALUES (?, ?, datetime("now"))',
        ...
        removeItem: function(key, callback) {
            ...
            self.sqlite.run({
                database: self.dbName,
                statement: 'DELETE FROM storage WHERE key = ?',
                values: [key]
            }).then(function() {
                if (callback) callback(null);
            }).catch(function(error) {
