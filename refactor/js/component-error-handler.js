    ...
    const ComponentErrorHandler = {
        ...
        activateFallback: function(wrapperId, componentName) {
            ...
            if (wrapper) {
                ...
                const fallback = wrapper.querySelector('.component-fallback');
                if (fallback) {
        ...
        scheduleRecovery: function(componentName, wrapperId, initFunction, context) {
            const self = this;
            const delay = window.StackMapSafeMode ? 10000 : 5000; // Longer delay in safe mode
        ...
        logError: function(componentName) {
            try {
                const count = parseInt(localStorage.getItem('stackmap_component_errors') || '0', 10);
                localStorage.setItem('stackmap_component_errors', (count + 1).toString());
                
                // Component-specific error count
                const componentCount = parseInt(localStorage.getItem(`stackmap_error_${componentName}`) || '0', 10);
